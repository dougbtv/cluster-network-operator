...
import (
	...
	netattachdeftypes "github.com/openshift/cluster-network-operator/pkg/controller/netattachdef/v1"
...
func newReconciler(mgr manager.Manager, status *statusmanager.StatusManager) *ReconcileNetAttachDef {
	configv1.Install(mgr.GetScheme())
	operv1.Install(mgr.GetScheme())
	return &ReconcileNetAttachDef{
...
func add(mgr manager.Manager, r *ReconcileNetAttachDef) error {

	netattachdeftypes.AddToScheme(scheme.Scheme)

	log.Printf("!bang ADDING NETATTACHDEF!?!?")
	// Create a new controller
	c, err := controller.New("netattachdef-controller", mgr, controller.Options{Reconciler: r})
	if err != nil {
		return err
	}

	// Watch for changes to NetworkAttachmentDefinition
	err = c.Watch(&source.Kind{Type: &netattachdeftypes.NetworkAttachmentDefinition{}}, &handler.EnqueueRequestForObject{})
	if err != nil {
		return err
	}
...
var _ reconcile.Reconciler = &ReconcileNetAttachDef{}

// Reconcile updates the state of the cluster to match that which is desired
func (r *ReconcileNetAttachDef) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	log.Printf("Reconciling network-attachment-definitions.k8s.cni.cncf.io %s\n", request.Name)

	// Fetch the CR instance
	netattachConfig := &netattachdeftypes.NetworkAttachmentDefinition{TypeMeta: metav1.TypeMeta{APIVersion: "v1", Kind: "NetworkAttachmentDefinition"}}
	err := r.client.Get(context.TODO(), request.NamespacedName, netattachConfig)
	if err != nil {
		return reconcile.Result{}, err
	}

	// log.Printf("!bang DEBUG: %s\n", reconcile.Result)
	return reconcile.Result{}, nil
}

// ReconcileNetAttachDef reconciles a Network.operator.openshift.io object
type ReconcileNetAttachDef struct {
...
// RefReconcileOperConfig is a reference from some original code.
type RefReconcileOperConfig struct {
	// This client, initialized using mgr.Client() above, is a split client
	// that reads objects from the cache and writes to the apiserver
	client client.Client
	scheme *runtime.Scheme
...
// // ReferenceReconcile DOUG'S REFERNECE updates the state of the cluster to match that which is desired
// // in the operator configuration (Network.operator.openshift.io)
// func (r *RefReconcileOperConfig) ReferenceReconcile(request reconcile.Request) (reconcile.Result, error) {
